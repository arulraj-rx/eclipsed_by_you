...
class DropboxToInstagramUploader:
    ...
    def verify_instagram_post_by_media_id(self, media_id, page_token):
        ...
        try:
            ...
            for attempt in range(10):
                ...
                if res.status_code == 200:
                    ...
                    try:
                        ...
                        self.log_console_only(f"❌ 400 Error details: {error_msg} (Code: {error_code})", level=logging.INFO)
                    except:
                        self.log_console_only(f"❌ 400 Error response: {res.text}", level=logging.INFO)
            ...
            # If verification fails, try alternative approach - check if post exists via feed
            self.log_console_only("🔄 Trying alternative verification via feed...", level=logging.INFO)
            return self.verify_instagram_post_alternative(media_id, page_token)
            
        except Exception as e:
            self.send_message(f"❌ Exception verifying Instagram post: {e}", level=logging.ERROR)
            return False

    def verify_instagram_post_alternative(self, media_id, page_token):
        """Alternative verification method using feed endpoint."""
        try:
            # Try to get recent media from the Instagram account
            ...
            self.log_console_only(f"📡 Alternative verification URL: {url}", level=logging.INFO)
            
            res = self.session.get(url, params=params)
            if res.status_code == 200:
                data = res.json()
                recent_media = data.get("data", [])
                
                # Check if our media_id is in the recent posts
                for media in recent_media:
                    if media.get("id") == media_id:
                        self.send_message(f"✅ Instagram post verified via alternative method!", level=logging.INFO)
                        self.log_console_only(f"📸 Found post in recent media: {media_id}", level=logging.INFO)
                        return True
                
                self.log_console_only(f"❌ Media ID {media_id} not found in recent posts", level=logging.INFO)
                return False
            else:
                self.log_console_only(f"❌ Alternative verification failed: {res.status_code}", level=logging.INFO)
                return False
                
        except Exception as e:
            self.log_console_only(f"❌ Exception in alternative verification: {e}", level=logging.INFO)
            return False
